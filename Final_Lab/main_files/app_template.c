...
#define TESTDNN // TESTDNN, GROUP0, ...
	...
	for(int i = 0; i < layers_num; i++){
		...
		while (!XUartPs_IsReceiveData(STDIN_BASEADDRESS)) {
			; // wait until first byte of the image is sent before starting the timer
		}
	...
	while (1){
		...
		printf("Reading the image took: %d Tics or %.5f sec\n", dtic_readImage, dt_readImage); //printf needed because printing floating point values
